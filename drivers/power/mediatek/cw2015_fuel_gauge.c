...
#define CW2015_ENABLE_LOG 0///1 //CHANGE   Customer need to change this for enable/disable log
#define CW2015_I2C_BUSNUM 2 //CHANGE   Customer need to change this number according to the principle of hardware
...
#define cw2015_printk(fmt, arg...)\
	({\
		if(CW2015_ENABLE_LOG){\
			printk("cw2015 : %s : " fmt, __FUNCTION__ ,##arg);\
		}else{}\
...
///////////////////////////////////////////
	0x16,0x2D,0x8A,0x79,0x61,0x48,0x44,0x59,
	...
	0x15,0x9F,0x4D,0x79,0x61,0x48,0x44,0x59,
	0x5B,0x4F,0x4D,0x53,0x5E,0x6E,0x65,0x57,
	0x4A,0x41,0x36,0x32,0x38,0x45,0x4B,0x54,
	0x5E,0x50,0x0B,0x85,0x19,0x32,0x6D,0x83,
	0x8B,0x8D,0x8F,0x8D,0x3B,0x17,0x78,0x16,
...
int cw2015_read_version(void)
{
	...
	while(i++ <4)
	{
		ret=cw_read(cw_bat->client, 0, &reg_val);
		if(ret >0)break;//return reg_val;
	}
	printk("ppppppppp--%d\n",reg_val);
	return reg_val;
}
int cw2015_read_all_reg(char *buf)
{
	int i=0,ret;
	u8 reg_val;
	printk("cw2015 read  all reg:\n");
		while(i <11)
		{
			ret=cw_read(cw_bat->client, i, &reg_val);
			buf[i]=reg_val;
			printk("[0x%x]=0x%x  ",i,reg_val);			
			i++;
			if(i==3 ||i==6 ||i==9)printk("\n");
		}
	printk("\n");
	return 0;
}
...
int check_charging_state(const char *filename)
{
	...
	cw2015_printk(" filename = %s  state = %s \n", filename, buf);
	filp_close(fp,NULL);
	set_fs(fs);
...
fs_initcall(cw2015_fgadc_init);   //use fs_initcall to make cw2015 registration earlier
