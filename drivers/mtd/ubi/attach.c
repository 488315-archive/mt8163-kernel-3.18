...
/**
 * ubi_check_pages_pattern - check if pages contain only a certain byte pattern.
 * @ubi: ubi device
 * @pnum: the number of peb to check
 * @page_from: the start index of page to check
 * @page_to: the end index of page to check
 * @patt: the pattern to check
 *
 * This function returns %1 in there are only @patt bytes in all pages, %0 if
 * something else was also found, and %-1 if read error occurred.
 */
int ubi_check_pages_pattern(const struct ubi_device *ubi, int pnum,
					int page_from, int page_to, uint8_t patt)
{
	int i = page_from;
	int err = 0;

	ubi_assert(page_from >= 0);
	ubi_assert(page_to >= 0);
	ubi_assert(page_to >= page_from);

	for (; i < page_to; i++) {
		err = ubi_io_read_oob(ubi, ubi->databuf, NULL, pnum,
							i * ubi->mtd->writesize);
		if (err < 0)
			return err;

		err = ubi_check_pattern(ubi->databuf, patt, ubi->mtd->writesize);
		if (!err)
			break;
	}
...
int ubi_backup_init_scan(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	if (backup_seb[0] != NULL && backup_seb[1] != NULL) {
		...
		/*if sqnum0 > sqnum1 + 2 means that, LEB0 has record a page which is over page_1,
		 so, no need to check the page_1's data in LEB1 of the backup volume.*/
		if (peb0 != -1 && (peb0 == peb1 || (sqnum0 - sqnum1) > 2)) {
		...
		for (i = 0; i < num; i++) {
			...
			if (source_page == 1) {
				...
				if (strncmp("VIDVIDVID", ubi->databuf, 9) == 0) {
					...
					err = ubi_check_pages_pattern(ubi, source_pnum, check_page, page_cnt, 0xFF);
					if (err == 1) {
						ubi_msg("Pages @ (%d) are all 0xFF.", source_pnum);
						corrupt = 2;
						break;
					}
					continue;
