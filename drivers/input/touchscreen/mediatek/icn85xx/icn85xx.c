...
int icnt_pc_tool_i2c_rxdata(char dev_addr, char *rxdata, int length)
{
	...
	int retries = 0;
			...
			struct i2c_msg msgs[] = {
					{
					.addr = dev_addr,// & I2C_MASK_FLAG) | I2C_DMA_FLAG,
					.flags = I2C_M_RD,
...
int icn85xx_prog_i2c_rxdata(unsigned int addr, char *rxdata, int length)
{
	...
	struct i2c_msg msgs[] = {
		{
			.addr = ICN85XX_PROG_IIC_ADDR,// & I2C_MASK_FLAG) | I2C_DMA_FLAG ,
			.flags = 0,
			//.timing = I2C_MASTER_CLOCK,
			.len = 3,
			.buf = tmp_buf,
...
int icn85xx_i2c_rxdata(unsigned short addr, char *rxdata, int length)
{
	...
	struct i2c_msg msgs[] =
	      {
		   {
			.addr = ICN85XX_NORMAL_IIC_ADDR,//i2c_client->addr,   //ICN85XX_NORMAL_IIC_ADDR
			.flags = 0,
			.len = 2,
			.buf = tmp_buf,
		   },
		   {
			.addr = ICN85XX_NORMAL_IIC_ADDR,//i2c_client->addr,
			.flags = I2C_M_RD,
			.len = length,
			.buf = rxdata,
		   },
		...
		if(retries >= I2C_RETRY_NUM)
		{
			...
			icn85xx_trace("%s i2c read successful: %d, rxdata_len = %d\n", __func__, ret, length);
		}	
		return ret;
	}
#endif
...
int i2c_read(struct i2c_client *client, u8 *rd_buf, int rd_len)	
{ 	
	int ret = 0; 	
	struct i2c_adapter *adap = client->adapter; 
	struct i2c_msg msg; 	
...
{	
	...
	int ret = -1;
	int i=0,j = 0,buf_check;
	unsigned char button;
	static unsigned char button_last;
	unsigned char tmp_buf[2];
	
	tmp_buf[0] = (unsigned char)(0x1000>>8);
	tmp_buf[1] = (unsigned char)(0x1000);
...
//---------------------TPD_SLIDE_WAKEUP------------------------------
	...
	ret = i2c_write(i2c_client, tmp_buf, 2);
	if(ret < 0)
	{
		icn85xx_trace("%s i2c_write failed: %d\n", __func__, ret);
		return ;
	}
#if 0
	ret = i2c_read(i2c_client, buf, POINT_NUM * POINT_SIZE + 2);
	if(ret < 0)
	{
		icn85xx_trace("%s read_data i2c_rxdata failed: %d\n", __func__, ret);
		return ;
    ...
    {
    		buf_check = 0;
				ret = i2c_read(i2c_client, buf, POINT_NUM * POINT_SIZE + 3);
		    if (ret < 0) {
	...
	//	icn85xx_trace("u8EventId %d\n", icn85xx_ts->point_info[i].u8EventId);
		...
		input_report_abs(tpd->dev, ABS_MT_POSITION_X, icn85xx_ts->point_info[i].u16PosX*1280/720);
		input_report_abs(tpd->dev, ABS_MT_POSITION_Y, icn85xx_ts->point_info[i].u16PosY*720/1280);
...
static int  tpd_remove(struct i2c_client *client)
{
	...
	if(icn85xx_i2c_dma_va)
	{
		dma_free_coherent(NULL, 255, icn85xx_i2c_dma_va, icn85xx_i2c_dma_pa);
